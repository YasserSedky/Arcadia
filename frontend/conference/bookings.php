...
if (!$user || empty($user['id'])) {
    header('Location: ' . APP_BASE . '/backend/auth/logout.php');
    exit;
}

$pdo = DB::conn();
$venue_id = (int)($_GET['venue_id'] ?? 0);
if ($venue_id < 1) redirect('/frontend/conference/index.php');

// fetch venue
$stmt = $pdo->prepare('SELECT * FROM conf_venues WHERE id = ?');
$stmt->execute([$venue_id]);
$venue = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$venue) redirect('/frontend/conference/index.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        try {
            ...
            redirect('/frontend/conference/mybookings.php?success=1');
