...
function fail(string $msg){
  redirect('/backend/auth/register.php?err=' . urlencode($msg));
}
...
try {
  ...
  if ($role_code === 'resident') {
    redirect('/backend/auth/login.php?ok=' . urlencode('تم إنشاء الحساب بنجاح. سيتم تفعيل حسابك بعد موافقة مدير السكن'));
  } else {
    redirect('/backend/auth/login.php?ok=' . urlencode('تم إنشاء الحساب بنجاح'));
  }
